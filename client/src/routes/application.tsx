...
export default function ApplicationDetail() {
  ...
  const { data, isLoading, isSuccess } = useQuery({
    ...
    queryFn: async () => {
      const result = await GET<ApplicationDetails>(`/api/applications/${id}`, null)
  ...
  async function save(obj: { [key: string]: string }) {
    const { error } = await PUT(`/api/applications/${data!.application.id}`, obj)
    if (error) {
      console.error(error)
  ...
  const saveSkills = async (skills: Skill[]) => {
    const { error } = await POST<any>(`/api/applications/${data!.application.id}/skills`, { skills })
    if (error) {
      console.error(error)
    ...
    <Base className="flex flex-col gap-4">
      ...
      <section className="grid grid-cols-12 gap-4">
        <div className="col-span-9 flex flex-col gap-6">
          <header>
            <InlineEditDropdown name="status" value={data!.application.status}
              onSave={save} options={statusOptions}
            />
            <h1 className="text-3xl mt-4">{data!.application.name}</h1>
            <span className="block text-muted-foreground">{data!.application.position}</span>
          </header>
          ...
          <Documents onUpdate={save} application={data.application} />
          <Actions applicationId={data.application.id} actions={data.actions} />
        </div>
        ...
        <div className="flex flex-col col-span-3 gap-2 border rounded-lg border-gray-200 p-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {renderDate(data.application.create_date)}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <a href={data.application.homepage} target="_blank" className="block text-sm text-muted-foreground hover:underline">
              {data.application.homepage}
            </a>
          </div>
          <div>
            <span className="text-md">Job post</span>
            <a href={data.application.link} target="_blank" className="block text-sm text-muted-foreground hover:underline">
